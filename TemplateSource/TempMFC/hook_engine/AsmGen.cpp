#include "stdafx.h"
#include "BaseHook.h"
#include "../hook_engine/asmjit/asmjit/src/asmjit/x86.h"
#include "../hook_engine/asmjit/asmtk/src/asmtk/asmtk.h"
#include "AsmGen.h"
...
bool AsmGen::AddCode(char *Instruction)
{
	...
	if (err)
	{
		printf("code init failed ins = %s err = %d\r\n", Instruction,err);
		//__debugbreak();
		return false;
	}
...
bool AsmGen::WriteStreamTo(BYTE *Buffer, size_t BufferSize)
{
	...
	DWORD old = 0;
	VirtualProtect((PVOID)Buffer, BufferSize, PAGE_EXECUTE_READWRITE,&old);
	memset(Buffer, 0, BufferSize);
	memcpy(Buffer, m_Stream.data(), m_Stream.size());
	VirtualProtect((PVOID)Buffer, BufferSize, old, &old);
	return true;
}