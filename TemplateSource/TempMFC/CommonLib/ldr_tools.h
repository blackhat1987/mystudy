...
namespace usr::ldr::tools
{
	...
	PVOID get_section_by_name(PVOID ImageBase, std::string sec_name);

	PVOID get_peb_ldr_by_hmodule(PVOID hmodule);

	PVOID get_module_by_hash(uint64_t _hash);

	PVOID get_api_by_hash(PVOID hmodule, uint64_t _hash);
};